...
def create_app(auth_code: str | None = None) -> Flask:
    ...
    def api_icloud_adapter_workspace(adapter_id: str):
        ...
        return Response(f"""<!doctype html><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>iCloud HTML 适配</title>
<style>
  :root{{color-scheme:light}}*{{box-sizing:border-box}}body{{font:13px system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;margin:0;background:#f4f6fb;color:#17202a;overflow:hidden}}
  header{{height:142px;padding:15px 20px;background:#fff;border-bottom:1px solid #e3e7f0;box-shadow:0 2px 10px rgba(27,39,75,.06);position:relative;z-index:2}}
  .title{{display:flex;align-items:center;gap:9px;font-size:14px;font-weight:750;color:#26334b}}.title-dot{{width:8px;height:8px;border-radius:50%;background:#596de0;box-shadow:0 0 0 4px #eef0ff}}
  .hint{{margin:7px 0 8px;color:#7d8799;font-size:11px}}.source{{overflow:hidden;color:#8a94a6;font-size:11px;text-overflow:ellipsis;white-space:nowrap}}.source a{{color:#5368d8;text-decoration:none}}
  .toolbar{{display:flex;align-items:center;gap:8px;margin-top:10px}}button{{min-height:29px;padding:0 11px;border:1px solid #dce2ef;border-radius:7px;background:#fff;color:#53617a;font:inherit;font-size:11px;font-weight:700;cursor:pointer}}button:hover{{border-color:#bfc9e8;background:#f7f8ff;color:#4056c6}}button.primary{{border-color:#5b6ee1;background:#5b6ee1;color:#fff}}button.primary:disabled{{opacity:.45;cursor:not-allowed}}button.danger{{margin-left:auto}}input{{width:285px;height:29px;padding:0 9px;border:1px solid #dce2ef;border-radius:7px;background:#fff;color:#263146;font:inherit;font-size:11px}}input:focus{{border-color:#8798ff;box-shadow:0 0 0 3px rgba(79,107,255,.10);outline:none}}#selected{{max-width:380px;overflow:hidden;color:#08785f;font:11px ui-monospace,SFMono-Regular,Menlo,monospace;text-overflow:ellipsis;white-space:nowrap}}#status{{color:#b45309;font-size:11px}}
  main{{height:calc(100vh - 142px);padding:12px}}iframe{{display:block;width:100%;height:100%;border:1px solid #d9dfeb;border-radius:8px;background:#fff;box-shadow:0 3px 12px rgba(27,39,75,.05)}}
  @media (max-width:720px){{header{{height:190px;padding:13px}}main{{height:calc(100vh - 190px);padding:8px}}.toolbar{{flex-wrap:wrap}}input{{flex:1;min-width:190px}}#selected{{max-width:220px}}}}
</style>
