...
开启“账号持久环境模式”后，所有 Roxy 入口共用关闭环境分配器：仅选择当前工作区/项目内 `openStatus=false/0` 的窗口；同一运行目录的多线程/进程通过 SQLite 文件锁互斥占用，任务结束只关闭不删除。环境不足时等待 `ROXY_IDLE_PROFILE_WAIT_TIMEOUT`（默认 300 秒），超时返回明确错误，不调用创建接口，也不强行接管已打开窗口。多台机器使用同一工作区时还应避免外部客户端同时手动操作；`forceOpen=false` 保留服务端打开冲突保护。

初始化顺序：检查关闭状态 → 取得互斥占用 → 再检查状态 → 保存可用指纹快照 → 分配新代理 → 清理本地/云端 Cookie、缓存及站点状态 → 恢复可用账号指纹或随机指纹 → 关闭历史同步、设置空白启动页 → 写入新代理 → 打开。仅补跑、查活、套餐及资格任务读取历史账号资料；注册和其他任务不读取历史账号指纹。代理不从账号历史记录恢复，固定池也会排除历史出口；池中没有其他出口时需补充代理。

新注册保存 Roxy API 可读的系统、UA、内核及指纹字段快照；旧账号优先使用已保存快照或仍存在的历史环境信息，缺失时随机初始化。供应商未公开的内部随机种子不作为可恢复字段。套餐/资格查询原本使用协议请求，仍保留账号 Session/device 信息；需要浏览器查活时使用同一复用入口。列表路径默认 `/browser/list`，较新 Roxy 可配置 `/browser/list_v3`。
...
在「配置 → 代理池 → API代理」设置「健康检查尝试次数（含首次）」，范围 1–20，默认 3。例如设为 5，最多尝试 5 次（首次 + 4 次重试）：每次重新请求 API，并检查其中一个未隔离的 IP，通过即停止。获取失败、空返回、重复隔离出口、健康检查失败均占用一次尝试；次数用尽后返回失败，不改用固定代理池。该次数与「API数量」无关，注册、换绑、查活及其他获取 API 代理的任务共用；查活只替换地区参数，不修改共享配置；API 尝试用尽后不再次走套餐路由获取代理，以免重置次数或使用注册地区。「注册任务检查健康IP」关闭时只重试获取代理。任务日志显示当前次数、上限及失败原因。保存后下一次代理选择生效，各任务独立计数。

此设置通过 `.env` 与运行时配置加载，macOS Intel/Apple Silicon、Docker ARM64/x86_64 和 Windows 共用同一实现，无新增平台依赖。
...
SMS_SERVICE = "openai"
SMS_COUNTRY = "国家代码"
SMS_MAX_PRICE = ""      # 可选最高价；留空不传 maxPrice，支持该参数的平台按此值取号
SMS_MAX_RETRIES = 10
SMS_CODE_WAIT = 120
SMS_POLL_INTERVAL = 5
...
#### API 出口诊断兼容（2026.09.13.4）

API日志分别记录提取数量和出口健康检查结果。HTTP代理在 curl CONNECT 传输失败时，诊断请求通过标准 HTTP/1.0 隧道对同一代理重试；不直连、不关闭证书验证、不把 403/挑战视为成功。兼容会话在检测结束后关闭。API管理的“检测出口”验证提取与出口连通，实际任务仍执行完整信誉、匿名性及业务入口检查。弹窗展示实际请求地区/会话/格式/数量，共用参数可能覆盖原始链接中的值；复制与打开均使用实际请求链接。
