...
「配置 → 邮箱 / OTP → iCloud 邮箱」还提供 HTML 接码地址适配器：输入并添加接码 URL 后，点击列表中的“开始适配”，在打开的页面里点击验证码元素并保存；如果商家禁止代理显示，也可以打开公网地址后手动填写 CSS 选择器。选择器按商家域名复用，同一域名后续更换邮箱、token 或路径时会自动使用已保存规则；删除适配记录后自动回退到通用六位验证码识别。
