...
def _job_traffic_fields(result: Any) -> dict[str, Any]:
    ...
    means every terminal task update persists the same per-job value that the
...
def _quarantine_browser_challenged_proxy(
    ...
    deleted = False
    if (
        str(getattr(_proxy_cfg, "PROXY_MODE", "pool") or "pool").strip().lower() != "api"
        and bool(getattr(_proxy_cfg, "PROXY_DELETE_UNHEALTHY_IPS", False))
    ):
