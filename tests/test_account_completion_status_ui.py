...
def test_account_table_exposes_clickable_link_and_sms_statuses():
...
def test_manual_link_sync_uses_shared_prompt_and_sync_api():
    source = TEMPLATE.read_text(encoding="utf-8")

    assert 'id="btnManualLinkStatusV2"' in source
    assert "手动同步提链状态" in source
    assert "每行一个" in source
    assert "confirmText: '同步'" in source
    assert "/api/accounts/link-status/sync" in source
